  ...
  public partial class pmessage : ForumPage
  {
    ...
    protected void Save_Click([NotNull] object sender, [NotNull] EventArgs e)
    {
      ...
      else
      {
        ...
        if (recipients.Count > YafContext.Current.BoardSettings.PrivateMessageMaxRecipients &&
            !YafContext.Current.IsAdmin && YafContext.Current.BoardSettings.PrivateMessageMaxRecipients != 0)
        {
          // to many recipients
          YafContext.Current.AddLoadMessage(
            this.GetTextFormatted("TOO_MANY_RECIPIENTS", YafContext.Current.BoardSettings.PrivateMessageMaxRecipients));
          return;
        }
        ...
        foreach (string recipient in recipients)
        {
          ...
          if ((LegacyDb.user_pmcount(userId.Value).Rows[0]["NumberTotal"].ToType<int>() >=
               LegacyDb.user_pmcount(userId.Value).Rows[0]["NumberAllowed"].ToType<int>()) && !YafContext.Current.IsAdmin &&
              !(bool)
               Convert.ChangeType(UserMembershipHelper.GetUserRowForID(userId.Value, true)["IsAdmin"], typeof(bool)))
          {
